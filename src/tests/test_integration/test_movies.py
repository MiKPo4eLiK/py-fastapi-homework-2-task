...
async def test_get_movies_empty_database(client) -> None:
    ...
    assert response_data["movies"] == [], "Expected empty movies list."
    assert response_data["total_items"] == 0, "Expected total_items to be 0."
...
async def test_page_exceeds_maximum(client, db_session, seed_database) -> None:
    ...
    count_stmt = select(func.count(MovieModel.id_))
...
async def test_movies_sorted_by_id_desc(client, db_session, seed_database) -> None:
    ...
    stmt = select(MovieModel).order_by(MovieModel.id_.desc()).limit(10)
    result = await db_session.execute(stmt)
    expected_movies = result.scalars().all()

    expected_movie_ids = [movie.id_ for movie in expected_movies]
...
async def test_get_movie_by_id_valid(client, db_session, seed_database) -> None:
    ...
    """
    stmt_min = select(MovieModel.id_).order_by(MovieModel.id_.asc()).limit(1)
    result_min = await db_session.execute(stmt_min)
    min_id = result_min.scalars().first()

    stmt_max = select(MovieModel.id_).order_by(MovieModel.id_.desc()).limit(1)
    result_max = await db_session.execute(stmt_max)
    max_id = result_max.scalars().first()

    random_id = random.randint(min_id, max_id)

    stmt_movie = select(MovieModel).where(MovieModel.id_ == random_id)
    ...
    assert (
        response_data["id"] == expected_movie.id_
...
async def test_get_movie_by_id_fields_match_database(
    ...
    response = await client.get(f"/api/v1/theater/movies/{random_movie.id_}/")
    assert (
        response.status_code == 200
    ), f"Expected status code 200, but got {response.status_code}"
    ...
    expected_date_str = random_movie.date.isoformat()
    actual_date_str = response_data[date_key].split("T")[0]

    assert response_data["id"] == random_movie.id_, "ID does not match."
    assert response_data["name"] == random_movie.name, "Name does not match."
    assert actual_date_str == expected_date_str, "Date does not match."
    ...
    assert (
        response_data["budget"] == float(random_movie.budget)
        if random_movie.budget is not None
        else None
    ), "Budget does not match."
    assert response_data["revenue"] == random_movie.revenue, "Revenue does not match."

    assert (
        response_data["country"]["id"] == random_movie.country.id_
    ...
    expected_genres = sorted(
        [{"id": genre.id_, "name": genre.name} for genre in random_movie.genres],
        key=lambda x: x["id"],
    )
    ...
    expected_actors = sorted(
        [{"id": actor.id_, "name": actor.name} for actor in random_movie.actors],
        key=lambda x: x["id"],
    )
    ...
    expected_languages = sorted(
        [{"id": lang.id_, "name": lang.name} for lang in random_movie.languages],
        key=lambda x: x["id"],
    )
...
async def test_create_movie_and_related_models(client, db_session) -> None:
    ...
    movie_data = {
        "name": "New Movie",
        "release_date": (dt_date.today() + timedelta(days=5)).isoformat(),
        "score": 85.5,
    ...
    expected_date_str = movie_data["release_date"]
    actual_date_str = response_data["date"].split("T")[0]

    assert response_data["name"] == movie_data["name"], "Movie name does not match."
    assert actual_date_str == expected_date_str, "Movie date does not match."
...
async def test_create_movie_duplicate_error(client, db_session, seed_database) -> None:
    ...
    date_iso = existing_movie.date.isoformat()
    ...
    response_data = response.json()
    expected_detail = f"A movie with the name '{movie_data['name']}' and release date '{movie_data['release_date']}' already exists."
    assert (
...
async def test_delete_movie_success(client, db_session, seed_database) -> None:
    ...
    stmt_check = select(MovieModel).where(MovieModel.id_ == movie_id)
...
async def test_update_movie_success(client, db_session, seed_database) -> None:
    ...
    update_data = {
        ...
        "score": 95.0,
        "release_date": (dt_date.today() + timedelta(days=10)).isoformat(),
    }
    ...
    expected_date_str = update_data["release_date"]
    actual_date_str = response_data["date"].split("T")[0]

    assert (
        response_data["name"] == update_data["name"]
    ), "Movie name in response was not updated."
    assert (
        response_data["score"] == update_data["score"]
    ), "Movie score in response was not updated."
    assert (
        actual_date_str == expected_date_str
    ), "Movie date in response was not updated."
    ...
    ), "Movie name was not updated in the database."
    assert (
        updated_movie.score == update_data["score"]
    ), "Movie score was not updated in the database."
    assert (
        updated_movie.date.isoformat() == update_data["release_date"]
    ), "Movie date was not updated in the database."
